...
一个完整的 gs-http-gen IDL 项目包含以下组成部分：

### 项目目录结构

一个标准的 IDL 项目应该包含：

1. **meta.json** - 项目元信息配置文件（必需）
2. **IDL 文件** - 以 `.idl` 为扩展名的接口定义文件（至少一个）
...
**meta.json** 是 IDL 项目的必需配置文件，位于项目根目录，用于存储项目的元信息。
没有此文件的项目将无法正确解析。
...
- `name`: 项目名称，用于生成代码中的包名或命名空间
- `version`: 项目版本号
- `description`: 项目描述信息

### IDL 文件

IDL 文件使用 `.idl` 扩展名，用于定义数据结构、接口和服务。
一个项目可以包含多个 IDL 文件，这些文件在同一个命名空间中，可以相互引用彼此定义的类型、枚举和常量。

#### 命名空间与类型引用

- 所有 IDL 文件共享同一个全局命名空间
- 在任一 IDL 文件中定义的类型、枚举、常量可以在其他 IDL 文件中直接引用
- 类型名称在整个项目中必须唯一，不允许在不同文件中定义同名类型
- 系统会自动解析跨文件的类型引用关系

#### 文件组织建议

虽然 IDL 文件可以相互引用，但仍建议按照功能对文件进行组织，例如：

- **通用类型文件**：存放项目中通用的基础类型、枚举定义
- **业务模块文件**：按业务领域划分，如用户模块、订单模块等
- **服务接口文件**：定义 RPC 接口和相关请求响应类型
- **错误码文件**：集中定义项目的错误码枚举
