...
func ParseDir(dir string) (Project, error) {
	...
	for _, doc := range files {
		for i := range doc.SSEs {
			sse := doc.SSEs[i]
			segments, err := pathidl.Parse(sse.Path)
			if err != nil {
				return Project{}, errutil.Explain(err, `failed to parse path %s`, sse.Path)
			}
			params := make(map[string]string)
			for _, seg := range segments {
				if seg.Type == pathidl.Static {
			...
			srcType, ok := GetType(files, sse.Request)
			if !ok {
				return Project{}, errutil.Explain(nil, "type %s is used but not defined", sse.Request)
			}
			for _, f := range srcType.Fields {
				if f.Binding == nil || f.Binding.From != "path" {
					continue
				...
				if _, ok = params[f.Binding.Name]; !ok {
					err = errutil.Explain(nil, "path parameter %s not found in request type %s", f.Binding.Name, sse.Request)
					return Project{}, err
				}
				params[f.Binding.Name] = f.Name
			}
			for k, s := range params {
				if s == "" {
					err = errutil.Explain(nil, "path parameter %s not found in request type %s", k, sse.Request)
					return Project{}, err
				}
			}
			sse.PathSegments = segments
			sse.PathParams = params
			doc.SSEs[i] = sse
		}
	}
...
// ExitSse_def handles SSE definitions, including request/response
// types and annotations.
func (l *ParseTreeListener) ExitSse_def(ctx *Sse_defContext) {
	var err error
	...
	if !IsPascal(r.Name) {
		panic(errutil.Explain(nil, "SSE name %s is not PascalCase in line %d", r.Name, r.Position.Start))
	}

	// Request
	r.Request = ctx.Sse_req().User_type().IDENTIFIER().GetText()
	if !IsPascal(r.Request) {
		panic(errutil.Explain(nil, "SSE request type %s is not PascalCase in line %d", r.Request, r.Position.Start))
	}
	if !strings.HasSuffix(r.Request, "Req") {
		panic(errutil.Explain(nil, "SSE request type %s does not end with \"Req\" in line %d", r.Request, r.Position.Start))
	}
	l.Document.UserTypes[r.Request] = struct{}{}

	// Response
	respType := ctx.Sse_resp().User_type()
	r.Response = respType.IDENTIFIER().GetText()
	if !IsPascal(r.Response) {
		panic(errutil.Explain(nil, "SSE response type %s is not PascalCase in line %d", r.Response, r.Position.Start))
	}
	if !strings.HasSuffix(r.Response, "Resp") {
		panic(errutil.Explain(nil, "SSE response type %s does not end with \"Resp\" in line %d", r.Response, r.Position.Start))
	}
	l.Document.UserTypes[r.Response] = struct{}{}

	// Annotations
	for _, aCtx := range ctx.Sse_annotations().AllAnnotation() {
		a := Annotation{
			Key: aCtx.IDENTIFIER().GetText(),
			Position: Position{
				Start: aCtx.GetStart().GetLine(),
	...
	path, ok := GetAnnotation(r.Annotations, "path")
	if !ok {
		panic(errutil.Explain(nil, `annotation "path" not found in sse %s`, r.Name))
	}
	if path.Value == nil {
		panic(errutil.Explain(nil, `annotation "path" value is nil in sse %s`, r.Name))
	}

	// Retrieve the "method" annotation
	method, ok := GetAnnotation(r.Annotations, "method")
	if !ok {
		panic(errutil.Explain(nil, `annotation "method" not found in sse %s`, r.Name))
	}
	if method.Value == nil {
		panic(errutil.Explain(nil, `annotation "method" value is nil in sse %s`, r.Name))
	}

	// Retrieve the "contentType" annotation
	ct, ok := GetAnnotation(r.Annotations, "contentType")
	if !ok {
		panic(errutil.Explain(nil, `annotation "contentType" not found in sse %s`, r.Name))
	}
	if ct.Value == nil {
		panic(errutil.Explain(nil, `annotation "contentType" value is nil in sse %s`, r.Name))
	}
	...
	connTimeout, ok := GetAnnotation(r.Annotations, "connTimeout")
	if !ok {
		panic(errutil.Explain(nil, `annotation "connTimeout" not found in sse %s`, r.Name))
	}
	if connTimeout.Value == nil {
		panic(errutil.Explain(nil, `annotation "connTimeout" value is nil in sse %s`, r.Name))
	}

	// Retrieve the "readTimeout" annotation
	readTimeout, ok := GetAnnotation(r.Annotations, "readTimeout")
	if !ok {
		panic(errutil.Explain(nil, `annotation "readTimeout" not found in sse %s`, r.Name))
	}
	if readTimeout.Value == nil {
		panic(errutil.Explain(nil, `annotation "readTimeout" value is nil in sse %s`, r.Name))
	}

	// Retrieve the "writeTimeout" annotation
	writeTimeout, ok := GetAnnotation(r.Annotations, "writeTimeout")
	if !ok {
		panic(errutil.Explain(nil, `annotation "writeTimeout" not found in sse %s`, r.Name))
	}
	if writeTimeout.Value == nil {
		panic(errutil.Explain(nil, `annotation "writeTimeout" value is nil in sse %s`, r.Name))
	}
	...
	r.ConnTimeout, err = strconv.Atoi(strings.Trim(*connTimeout.Value, `"`))
	if err != nil {
		panic(errutil.Explain(nil, "invalid connTimeout value in sse %s", r.Name))
	}
	r.ReadTimeout, err = strconv.Atoi(strings.Trim(*readTimeout.Value, `"`))
	if err != nil {
		panic(errutil.Explain(nil, "invalid readTimeout value in sse %s", r.Name))
	}
	r.WriteTimeout, err = strconv.Atoi(strings.Trim(*writeTimeout.Value, `"`))
	if err != nil {
		panic(errutil.Explain(nil, "invalid writeTimeout value in sse %s", r.Name))
	}

	l.Document.SSEs = append(l.Document.SSEs, r)
}
